#include "../head/ADMMConst1.h"
#define MAX(X, Y) X * (X >= Y) + Y * (Y > X)


ADMMConst1::ADMMConst1() : MethodP2P()
{
#if DEBUG_CONSTRUCTOR
	std::cout << " ADMMConst1 Constructor" << std::endl;
...
ADMMConst1::ADMMConst1(float rho) : MethodP2P()
{
#if DEBUG_CONSTRUCTOR
	std::cout << "default ADMMConst1 Constructor" << std::endl;
#endif // DEBUG_CONSTRUCTOR
	_name = NAME;
	_rho = rho;
...
ADMMConst1::~ADMMConst1()
{
}
void ADMMConst1::setParam(float rho)
{
	_rho = rho;
}

void ADMMConst1::setTau(float tau)
{
	if (tau < 1) {
...
void ADMMConst1::solve(Simparam* result, const Simparam& sim, const StudyCase& cas)
{
#ifdef DEBUG_SOLVE
	cas.display();
	sim.display(1);
...
void ADMMConst1::updateP0(const StudyCase& cas)
{
	_id = _id + 1;
#ifdef INSTRUMENTATION
...
void ADMMConst1::init(const Simparam& sim, const StudyCase& cas)
{
...
void ADMMConst1::updateGlobalProb() {
	...
	updateCp2(&Cp2, _rho1, &Kappa1, &Kappa2, &G, &tempL1, &Qpart, &nVoisin, _nLine, _nAgent);
	Cp.add(&Cp1, &Cp2);
	// FB 3c
	updateLambda();
	updateBt1();
...
void ADMMConst1::updateLocalProb() {
	// FB 1a
...
void ADMMConst1::updateBp1()
{
	Bp1.add(&MU, &Tmoy);
}

void ADMMConst1::updateTl()
{
...
float ADMMConst1::updateResBis(MatrixCPU* res, int iter, MatrixCPU* tempNN)
{
	for (int t = 0; t < _nTrade; t++) {
		int k = CoresLinTrans.get(t, 0);
...
void ADMMConst1::updateMU()
{
	MU.add(&Tmoy);
	MU.subtract(&P);
}

void ADMMConst1::updateQ()
{
