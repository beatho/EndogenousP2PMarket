...
void OPFADMMGPU::updateP0(const StudyCase& cas)
{
	...
	initDFSPQ << <1, _nBus, _nBus* (8*sizeof(bool) + sizeof(int)) >> > (X._matrixGPU, nChild._matrixGPU, Childs._matrixGPU, _indiceBusBegin._matrixGPU, _indiceChildBegin._matrixGPU, _nBus);
...
void OPFADMMGPU::init(const Simparam& sim, const StudyCase& cas)
{
	...
	nAgentByBusCPU = cas.getNagentByBus();
	...
	if (nAgentMaByBus > _blockSizeSmall / 2) {
		std::cout << nAgentMaByBus << " " << _blockSizeSmall / 2 << std::endl;
	...
	nAgentByBusCPU.increment(0, 0, -1);
	...
	indiceBusBeginCPU = MatrixCPU(_nBus, 1);
	...
	VoltageLimitCPU = MatrixCPU(2, _nBus, 0); // min, max
...
void OPFADMMGPU::updateXWOCurrentOnCPU()
{
	X.transferCPU();
	Chat.transferCPU();
	PnMoy.transferCPU();
	...
	for (int bus = 0; bus < _nBus; bus++) {
		int begining = indiceBusBeginCPU.get(bus, 0);
		X.set(begining + 4, 0, PnMoy.get(bus, 0) * nAgentByBusCPU.get(bus,0));
		X.set(begining + 5, 0, PnMoy.get(bus + _nBus, 0)* nAgentByBusCPU.get(bus, 0));
	}
...
void OPFADMMGPU::display() {
	...
	Pn.transferGPU();
	_indiceBusBegin.transferGPU();
	_nAgentByBus.transferGPU();
	VoltageLimitReal.transferGPU();
	Pmin.transferGPU();
	Pmax.transferGPU();
	PnTmax.transferGPU();
	PnTmin.transferGPU();
		...
		if (nAgentShared == 1) { // cas trivial s'il n'y a qu'un agent, la divergence est entre les blocs donc c'est ok
			...
			for (iter = 0; iter < nIterLMax; iter++) {
				...
				if (thIdx < nAgentShared) {
					...
					m = Pnlocal[1] - moy + p - MULOCAL; // Pn.get(i + _nAgent, 0) - PnMoy.get(bus + _nBus, 0) + PnTilde.get(bus + _nBus, 0) - MuL.get(bus + _nBus, 0);
