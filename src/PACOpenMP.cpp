...
PACOpenMP::PACOpenMP() : MethodP2P()
{
	...
	std::cout << " PACOpenMP Constructor" << std::endl;
#endif // DEBUG_CONSTRUCTOR
	_name = NAME;
}


PACOpenMP::PACOpenMP(float rho) : MethodP2P()
{
#if DEBUG_CONSTRUCTOR
	std::cout << "default PACOpenMP Constructor" << std::endl;
#endif // DEBUG_CONSTRUCTOR
	_name = NAME;
	_rho = rho;
}
...
void PACOpenMP::setParam(float rho)
{
	_rho = rho;
}

void PACOpenMP::setGamma(float gamma)
{
	_gamma = gamma;
	if (_gammahat > _gamma) {
...
void PACOpenMP::setGammahat(float gammahat)
{
	_gammahat = gammahat;
	if (_gammahat > _gamma) {
		std::cout << "Warning : gammahat should be smaller than gamma";
...
void PACOpenMP::setInitCoef(float alpha, float phi, float theta)
{
...
void PACOpenMP::setBestRhoGamma(float lambdaMax, float lambdaMin, const StudyCase& cas)
{
...
void PACOpenMP::setBestRhoGammaHeuristic(const StudyCase& cas)
{
	int N = cas.getNagent();
	int Y = cas.getNCons();
	double lambdaMax = 1;
...
void PACOpenMP::solve(Simparam* result, const Simparam& sim, const StudyCase& cas)
{
#ifdef DEBUG_SOLVE
	cas.display();
	sim.display(1);
...
void PACOpenMP::updateP0(const StudyCase& cas)
{
	_id = _id + 1;
#ifdef INSTRUMENTATION
...
void PACOpenMP::init(const Simparam& sim, const StudyCase& cas)
{
	DELETEA(tempM1);
	DELETEA(tempM);
	DELETEA(X);
	...
	if (isAC) {
		...
		if (_nTradeQ != (_nAgentTrue * (_nAgentTrue - 1))) {
			std::cout << "err PACOpenMP : " << _nAgent << " " << _nAgentTrue << " " << _nTrade << " " << _nTradeP << " " << _nTradeQ << std::endl;
	...
	//_sizePACOpenMP = _nAgent + 2 * _nTrade;
...
void PACOpenMP::updateGlobalProb() {
...
void PACOpenMP::updateLocalProb() {
	// FB 1a
#ifdef INSTRUMENTATION
	std::chrono::high_resolution_clock::time_point t1 = std::chrono::high_resolution_clock::now();
#endif // INSTRUMENTATION
...
float PACOpenMP::updateRes(int indice)
{
	float resS = 0;
	float resR = 0;
	float resV = 0;
