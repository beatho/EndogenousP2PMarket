...
void System::solveIntervalle(std::string path, std::string name, MatrixCPU* interval)
{
	...
	{
		// juste une méthode où ce n'est pas implementé
	}
		
	
	//_case.display();
	//resetMethod(); grrrrrrrrrrrrrrrrrrrrrrrrr problème avec les matrices qui reste sur GPU
	//display(1);
	float epsG = _simparam.getEpsG();
	clock_t t = 0; 
	std::cout << "-";
...
void System::setMethod(std::string nameMethode) {
	...
	else if ((!nameMethode.compare(sOPFPDIPM))) {
			_methode = new OPFPDIPM;
	}
	else {
...
void System::displayTradesAgent()
{
	...
	for (int n = 0; n < nCons; n++) {
		...
		for (int i = nCons; i < N; i++) {
			if (Trade.get(i, n) > 0.0001) { // positif car on regarde ce que le générateur vend
