#include "../head/ADMMGPUConst1.cuh"
#define MAX(X, Y) X * (X >= Y) + Y * (Y > X)



ADMMGPUConst1::ADMMGPUConst1() : MethodP2P()
{
#if DEBUG_CONSTRUCTOR
	std::cout << "Constructeur ADMMGPUConst1" << std::endl;
#endif // DEBUG_CONSTRUCTOR
	_name = NAME;
}


ADMMGPUConst1::ADMMGPUConst1(float rho) : MethodP2P()
{
#if DEBUG_CONSTRUCTOR
	std::cout << "Constructeur ADMMGPUConst1 defaut" << std::endl;
#endif // DEBUG_CONSTRUCTOR
	_name = NAME;
	_rho = rho;
}

ADMMGPUConst1::~ADMMGPUConst1()
{
}

void ADMMGPUConst1::setParam(float rho)
{
	_rho = rho;
}

void ADMMGPUConst1::setTau(float tau)
{
	if (tau < 1) {
...
void ADMMGPUConst1::init(const Simparam& sim, const StudyCase& cas)
{
...
void ADMMGPUConst1::updateP0(const StudyCase& cas)
{
	_id = _id + 1;
...
void ADMMGPUConst1::solve(Simparam* result, const Simparam& sim, const StudyCase& cas)
{
#ifdef DEBUG_SOLVE
	cas.display();
	sim.display(1);
	...
	// à changer à la main pour l'instant
...
void ADMMGPUConst1::updateGlobalProbGPU()
{
	...
	updateAlpha << < _numBlocksNL, _blockSize >> > (alpha._matrixGPU, G._matrixGPU, Pn._matrixGPU, _nLine, _nAgent);
	updateQpart << < _nLine, _blockSize, _nAgent * sizeof(float) >> > (Qpart._matrixGPU, alpha._matrixGPU, _nAgent);
	updateQtot << <_numBlocksL, _blockSize >> > (Qtot._matrixGPU, Qpart._matrixGPU, alpha._matrixGPU, _nLine, _nAgent);
	...
	switch (_blockSize) {
	case 512:
		//updateCp2GPU<512> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		updateCp2a<512> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		updateCp2b<512> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case 256:
		updateCp2GPU<256> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<256> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<256> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case 128:
		updateCp2GPU<128> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<128> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<128> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case 64:
		updateCp2GPU<64> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<64> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<64> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case 32:
		updateCp2GPU<32> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<32> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<32> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case 16:
		updateCp2GPU<16> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<16> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<16> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case  8:
		updateCp2GPU<8> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<8> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<8> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case  4:
		updateCp2GPU<4> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<4> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<4> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case  2:
		updateCp2GPU<2> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<2> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<2> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		
		break;
	case  1:
		updateCp2GPU<1> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, nVoisin._matrixGPU, _rho1, _nLine, _nAgent);
		//updateCp2a<1> << <numBlocks, _blockSize >> > (Cp2._matrixGPU, tempL1._matrixGPU, G._matrixGPU, _nLine, _nAgent);
		//updateCp2b<1> << <numBlocks, _blockSize >> > (tempN1._matrixGPU, G._matrixGPU, Qpart._matrixGPU, _nLine, _nAgent);
		break;
	}
...
float ADMMGPUConst1::updateRes(MatrixCPU* res, MatrixGPU* Tlocal, int iter, MatrixGPU* tempNN)
{
	//
...
float ADMMGPUConst1::calcRes(MatrixGPU* Tlocal, MatrixGPU* P)
{
	float d1 = Tlocal->max2(&Tlocal_pre);
	float d2 = P->max2(&Tmoy);
	/*if (d1 < _mul * d2) {
