#include "../head/TestADMMGPUConst2.cuh"
#define NSTEPLOCAL 5
...
int testADMMGPUConst2()
{
	...
	if (!testADMMGPUConst2Contruct1()) return n;
	n++;
	if (!testADMMGPUConst2Contruct2()) return n;
	n++;
	if (!testADMMGPUConst2Contruct3()) return n;
	n++;
	if (!testADMMGPUConst2LAMBDA()) return n;
	n++;
	if (!testADMMGPUConst2Bt1()) return n;
	n++;
	if (!testADMMGPUConst2TradeP()) return n;
	n++;
	if (!testADMMGPUConst2UpdateRes()) return n;
	n++;
	if (!testADMMGPUConst2CalcRes()) return n;
	n++;
	if (!testADMMGPUConst2Solve1()) return n;
	n++;
	if (!testADMMGPUConst2Solve2()) return n;
	n++;
...
void testADMMGPUConst2Time(int test) {

	switch (test)
	{
	case 0:
		testADMMGPUConst2TimeLAMBDA();
		break;
	case 1:
		testADMMGPUConst2TimeBt1();
		break;
	case 2:
		testADMMGPUConst2TimeTradeP();
		break;
	case 3:
		testADMMGPUConst2TimeUpdateRes();
		break;
	case 4:
		testADMMGPUConst2TimeCalcRes();
		break;
	default:
...
bool testADMMGPUConst2TradeP()
{
	...
	updateTradePGPULocal<256> <<<nAgent, blockSize >> > (Tlocal._matrixGPU, Tlocal_pre._matrixGPU, Tmoy._matrixGPU, P._matrixGPU, MU._matrixGPU, nVoisin._matrixGPU, at1, at2, Bt1._matrixGPU, Ct._matrixGPU,
...
void testADMMGPUConst2TimeTradeP() {
	std::string fileName = "TempsTradeP9_F5.csv";
	std::chrono::high_resolution_clock::time_point a;
	std::chrono::high_resolution_clock::time_point b;
	unsigned int time;
	std::ios_base::openmode mode = std::fstream::in | std::fstream::out | std::fstream::app;
	...
	for (int i = 0; i < nNAgent; i++) {
		...
		for (int simu = 0; simu < nSimu; simu++) {
			...
			for (int repet = 0; repet < nRepet; repet++) {
				...
				cudaDeviceSynchronize();
				a = std::chrono::high_resolution_clock::now();
				updateTradePGPULocal<256> << <_n, blockSize >> > (TlocalCopy._matrixGPU, Tlocal_preCopy._matrixGPU, TmoyCopy._matrixGPU, PCopy._matrixGPU,
...
void testADMMGPUConst2TimeUpdateRes() {
	std::string fileName = "TempsResG9.csv";
...
void testADMMGPUConst2TimeCalcRes() {
	std::string fileName = "TempsResL9.csv";
